...
本文接 《API 配置（二）之服务提供者》，分享**服务消费者**相关的配置。
...
- 第 13 行：调用`#checkDefault()`方法，读取属性配置( 环境变量 + properties 属性 )到 ConsumerConfig 对象。
...
- 第 23 至 32 行：普通接口的实现。
  ...
  - 第 31 行：调用`checkInterfaceAndMethods(interfaceClass, methods)`
...
- 第 146 至 164 行：调用 MethodConfig 对象数组，添加到`map`中。
  ...
  - 第 160 行：调用`#appendAttributes(parameters, config, prefix)`方法，将
    ...
    配置对象的属性，添加到参数集合。在《API 配置（一）之应用》有详细解析。
...
- 第 177 行：添加到
  `com.alibaba.dubbo.rpc.StaticContext`进行缓存。
